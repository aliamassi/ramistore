<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
    <div class="aspect-square bg-gray-100 overflow-hidden">
        <img src="{{  isset($item->getMedia('products')[0])?$item->getMedia('products')[0]->getFullUrl():'' }}"
             data-image="{{ $item->getFirstMediaUrl() }}"
             alt="{{ $item->name }}"
            ...
            @else
                ...
                <div x-data="{ modalOpen: false }">
                    ...
                    <div x-show="modalOpen" x-cloak>
                        <x-product-modal :product="$item" @close="modalOpen = false" />
                    </div>
                </div>
            @endif
        </div>
    ...
    <button
            data-id="{{ $item->id }}"
            type="button"
            class="cart-add w-full text-center text-sm text-gray-600 hover:text-emerald-700 py-2 border-t"
    >
        Click card to add
    </button>
</div>
